...
- `omarchy-kids-set-tier` also hides the Omarchy top bar for tier mini (`omarchy-toggle-bar on` — the flag name is inverted, `on` enables the "bar is off" state, verified empirically) — found during setup-wizard testing (2026-08-29) that overriding Hyprland's keybindings doesn't touch the bar's own `omarchy.menu` icon, which still opens the normal Omarchy menu (terminal, file manager, ...) by mouse/touch regardless of what's keybound, undermining the rest of the lockdown. Since hiding the bar also removes its power-off button, `Launcher.qml` now carries its own icon-only "Ausschalten" tile with a two-step (✕/✓) confirm — the active local session can power off without admin rights via systemd-logind's default policy, no sudo needed.
